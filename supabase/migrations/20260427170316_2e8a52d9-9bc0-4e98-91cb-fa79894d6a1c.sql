...
CREATE POLICY "System owners can manage mpesa config" ON public.mpesa_config FOR ALL TO authenticated USING (public.has_role(auth.uid(), 'system_owner'::public.app_role)) WITH CHECK (public.has_role(auth.uid(), 'system_owner'::public.app_role));
...
CREATE POLICY "Members can create mpesa transactions" ON public.mpesa_transactions FOR INSERT TO authenticated WITH CHECK (public.is_business_member(auth.uid(), business_id));
